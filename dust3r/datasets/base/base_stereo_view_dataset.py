...
class BaseStereoViewDataset (EasyDataset):
    ...
    def __getitem__(self, idx):
        ...
        assert len(views) == self.num_views, f"Expected {self.num_views} views, got {len(views)} for idx={idx} ar_idx={ar_idx} with resolution={resolution}"
...
def transpose_to_landscape(view):
    ...
    if width < height:
        ...
        assert view['valid_mask'].shape == (height, width), f"Invalid valid_mask shape: {view['valid_mask'].shape} {height}, {width}"
        view['valid_mask'] = view['valid_mask'].swapaxes(0, 1)

        assert view['depthmap'].shape == (height, width), f"Invalid depthmap shape: {view['depthmap'].shape} {height}, {width}"
