...
def sample_gt_point_cloud(pts3d, valid_mask,num_pts, image=None):
        ...
        valid_image_first = image[valid_mask[:,:,0]]
        ...
        if n_vpts3d_first >= num_pts:
            ...
            # If image_pts3d is provided, filter the visible points to those within the camera frustum
            # store the rgb value for each point
            data['pcd_eval_visible_rgb'] = valid_image_first[perm[:num_pts]]
        else:
            perm = torch.randint(0, n_vpts3d_first, (num_pts,))
            data["pcd_eval_visible"] = valid_pts3d_first[perm]    
            data['pcd_eval_visible_rgb'] = valid_image_first[perm]

        return data


class SCRREAM(BaseStereoViewDataset):
    def __init__(self, *args, ROOT, data_path=None, train_list_path=None, test_list_path=None, n_ldi_layers=10,
                 num_pts=100000, max_pts=20000, input_n=1, enforce_img_reso_for_eval=[512,512], **kwargs):
    ...
    def adjust_resolution(self, image_ori, ldi, intrinsic):
        ...
        image, ldi, intrinsic = self.resize_and_completion(image_ori, ldi, intrinsic, self.resolution_lari[0], self.resolution_lari[1])
        if isinstance(self.enforce_img_reso_for_eval, list):
            ...
            image_input, _, _ = self.resize_and_completion(image_ori, None, None, 
                                                        self.enforce_img_reso_for_eval[0], self.enforce_img_reso_for_eval[1])
            
        return image_input, ldi, intrinsic, image
    ...
    def _get_views(self, idx, resolution, rng):
        ...
        rgb_image, ldi, intrinsics, image_pts3d = self.adjust_resolution(rgb_image, ldi, intrinsics)

        pts3d_lari, mask = ldi_to_pts3d(ldi, intrinsics)
        lari_data = sample_gt_point_cloud(pts3d_lari, mask, self.max_pts, image_pts3d)
        ...
        depthmap = np.ones((self.enforce_img_reso_for_eval[0], self.enforce_img_reso_for_eval[1]), dtype=np.float32)
        ...
        view_data = dict(
            ...
            label=obj_path,
            instance=f'{self._data_list[idx]}',
            ...
            pcd_eval_visible_rgb=lari_data['pcd_eval_visible_rgb'].astype(np.float32),
        )
